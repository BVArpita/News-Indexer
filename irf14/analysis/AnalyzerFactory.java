...
import edu.buffalo.cse.irf14.analysis.Content;
...
public class AnalyzerFactory {
	...
	public static AnalyzerFactory getInstance() {
		...
		return af!=null?af:new AnalyzerFactory();
	...
	public Analyzer getAnalyzerForField(FieldNames name, TokenStream stream) {
		...
		if(name==FieldNames.CATEGORY)
			return new Category(stream);
		else if(name==FieldNames.AUTHOR)
			return new Author(stream);
		else if(name==FieldNames.CONTENT)
			return new Content(stream);
		else if(name==FieldNames.PLACE)
			return new Place(stream);
		//System.out.println("Somethings wrong in analyzer!");	
		return null;
	}
}
