...
public class StemFilter extends TokenFilter {
	...
	public void processStream()  {
		if (stream != null) {
			String tknstr;
			Stemmer s;
			Token tkn=stream.next();
			while (stream.hasNext()) { 
					tknstr= stream.next().getTermText(); //read next token
			if (tknstr!= null) {
				if (isLetters(tknstr)) {
				s = new Stemmer();
				for (char c: tknstr.toCharArray()) {
							s.add(c);
				
				}
						s.stem();
				//		stream.previous(); //move token back as we need to change last read token
						tkn.setTermText(s.toString()); //change value, no iterator move
	...
	private boolean isLetters(String token) {
